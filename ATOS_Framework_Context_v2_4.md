...
*S1 patch release. Incorporates seven blocking specification gaps identified in the v2.3 structural integrity audit. Architecture and design principles unchanged. Research Foundation unchanged. Revision history removed for external/clean-handoff use.*
...
**Self-reference and new artifacts.** When the self-reference output is a *new artifact* (audit, review, critique, smoke-test result, analysis) rather than a revision of the framework document itself, standard Preamble Policy applies to the new artifact — a short-form preamble is prepended on external handoff. The framework document does not serve as the preamble in this case because the new artifact is structurally separate from the framework.
